  #Creates a mock global environment.
  #
  #Used by JS tests and JS doc examples.

  Rally.env.MockGlobal = {
    requires: ['Rally.auth.UserPermissions'],
    setupEnvironment: (contextConfig, environmentCls) ->
      if !environmentCls
        environmentCls = Rally.env.Environment
      Rally.environment = new environmentCls(context: this.setupContext(contextConfig))
      Rally.env.IoProvider.setSecurityToken('this-is-a-dummy-token')
    setupContext: (config) ->
      context = _.extend(this.getDefaultContext(), config)
      new Rally.env.Context(context)
    getDefaultContext: ->
      return {
        scope: {
          up: false,
    ...
    getDefaultUser: (config) ->
      new Rally.domain.User(
        _.extend({
          UserName: 'test user',
    ...
    setup: (spec) -> this.setupEnvironment()
  }
